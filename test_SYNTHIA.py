    ...
    with tf.Session(config=config) as sess:
        ...
        for t in range(0, len(test_files_list), FLAGS.batch_size):
            ...
            for b in range(FLAGS.batch_size):
                ...
                masked_pred_depth = cv2.resize(masked_pred_depth, dsize=( gt_width, gt_height),interpolation=cv2.INTER_NEAREST)  # sfmlearner increase the size to match the gt_depth
